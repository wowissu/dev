...
export type UnionToAllPossibleTuples<T, U = T, N extends number = 15> = T extends any ?
  Cons<T, Exclude<U, T> extends infer X ? {
    0: [], 1: UnionToAllPossibleTuples<X, X, Prev[N]>,
  }[[X] extends [never] ? 0 : 1] : never> :
  never;

export type NoRepeats<T extends readonly unknown[]> = { [M in keyof T]: { [N in keyof T]:
